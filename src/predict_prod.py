...
# Читаем URI DagsHub (если есть), иначе используемлокальную абсолютную БД
tracking_uri = os.getenv("MLFLOW_TRACKING_URI", sqlite_uri)
mlflow.set_tracking_uri(tracking_uri)
print(f"Подключаемся к реестру: {tracking_uri}")
# Используем синтаксис MLflow 3.x (Алиасы вместоСтадий)
model_name = "Iris_RF_Model"
alias = "champion"  # Заменили "Production" на алиас
"champion"
# Новый синтаксис: @ вместо /
model_uri = f"models:/{model_name}@{alias}"
print(f"Скачиваем модель по URI: {model_uri}")
# Скачиваем боевую модель из реестра по её Алиасу (безпутей к файлу!)
